...
def train_pose2rot(cfg):
    ...
    # Optional linear LR warmup (opt-in via config train.warmup_steps; default 0 = off,
    # preserves prior behavior). Stabilizes large-batch / linearly-scaled LR at startup
    # (Goyal et al. 2017): Adam's normalized update at full LR from step 0 can destabilize.
    base_lr = lr
    warmup_steps = int(train_cfg.get("warmup_steps", 0))
    if warmup_steps > 0 and is_main_process():
        logger.info(f"LR warmup: linear 0 -> {base_lr:g} over {warmup_steps} optimizer steps")
    ...
    # bfloat16 autocast (below) has the same exponent range as fp32, so loss
    # scaling is unnecessary and GradScaler is disabled. fp16 AMP here diverged
    # to NaN weights (bounded inputs; internal transformer overflow) -> bf16 fix.
    ...
    # Optional fk_wt ramp (captured ONCE so overwriting weight['fk_wt'] per-epoch is safe):
    # start gentle (fk_wt_start) so the anti-collapse tvar can break the collapse first,
    # then ramp linearly to the END value (weight['fk_wt']) over [start,end] epochs so the
    # FK-position loss refines the residual offset once rotations are learned. A strong
    # constant fk from scratch suppresses the collapse-break (it makes the model cheat
    # positions with constant rotations). Off when fk_wt_start==fk_wt or no ramp range.
    fk_end_wt = float(train_cfg["weight"].get("fk_wt", 0.0))
    fk_start_wt = float(train_cfg["weight"].get("fk_wt_start", fk_end_wt))
    fk_ramp_r0 = int(train_cfg["weight"].get("fk_ramp_start_epoch", 0))
    fk_ramp_r1 = int(train_cfg["weight"].get("fk_ramp_end_epoch", 0))
    fk_ramp_on = (fk_end_wt > 0 and fk_start_wt != fk_end_wt and fk_ramp_r1 > fk_ramp_r0)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if fk_ramp_on:
            if epoch < fk_ramp_r0:
                cur_fk = fk_start_wt
            elif epoch >= fk_ramp_r1:
                cur_fk = fk_end_wt
            else:
                cur_fk = fk_start_wt + (fk_end_wt - fk_start_wt) * (epoch - fk_ramp_r0) / float(fk_ramp_r1 - fk_ramp_r0)
            train_cfg["weight"]["fk_wt"] = cur_fk
            if is_main_process():
                logger.info(f"epoch {epoch + 1}: fk_wt ramp -> {cur_fk:.3f}")
