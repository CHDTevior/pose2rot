...
def rot6d_to_fk_positions_correct(rot6d, fk_offsets, parents):
    """Correct FK joint positions for the FK-position loss.

    Uses the VALIDATED convention (matches pose2rot_qa.py FK, self-test ~2e-7):
      rotation_6d_to_matrix (ROW convention, transforms3d) + bvh_forward (transl=0).
    The older rot6d_to_fk_positions is buggy for this purpose: rot6d_to_rotmat_tensor uses
    the column/dim=-1 convention (= transpose M^T), and it is fed the root-subtracted offset_a;
    FK(GT)!=position there, so minimizing that loss corrupts the model.

    Args:
        rot6d:      [B, T, J, 6] predicted 6D rotations.
        fk_offsets: [B, J, 3]    parent-local bone offsets / gscale, NO root-subtract
                                 (loader "fk_offset" = bvh.load().offsets / gscale).
        parents:    [B, J]       parent index per joint (root=-1).
    Returns:
        [B, T, J, 3] root-centered FK joint positions (joint-0 subtracted).
    """
    from utils.transforms3d import rotation_6d_to_matrix
    pred_positions = []
    for r6d, offset, parent in zip(rot6d, fk_offsets, parents):
        Rmat = rotation_6d_to_matrix(r6d)  # [T, J, 3, 3], row convention
        transl = torch.zeros((Rmat.shape[0], 3), device=Rmat.device, dtype=Rmat.dtype)
        _, pos = bvh_forward(Rmat, transl, offset, parent)  # [T, J, 3]
