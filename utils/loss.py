...
from .rotation import rot6d_to_fk_positions, rot6d_to_fk_positions_correct, rot6d_to_rotmat_batch
...
def compute_rot_loss(model_out, batch, weight_cfg, rot_criterion, vel_criterion, acc_criterion):
    ...
    if weight_cfg["fk_wt"] > 0:
        # CORRECT FK (rotation_6d_to_matrix row-convention + raw parent-local fk_offset);
        # the old rot6d_to_fk_positions(... offset_a) had FK(GT)!=position and corrupted the model.
    ...
    # anti-collapse demeaned-temporal loss: match the TIME-VARYING part of the motion
    # (subtract per-clip temporal mean), so a frozen/constant prediction is penalized
    # WITH a real gradient (d(x-mean_T x)/dx = I - 1/T != 0 even at constant pred) and
    # the model must reproduce GT's actual temporal pattern (not gameable by adding
    # noise energy, unlike a std-magnitude term which also has zero grad at constant).
    if weight_cfg.get("tvar_wt", 0.0) > 0:
        pred_dm = pred_rot6d - pred_rot6d.mean(dim=1, keepdim=True)
        gt_dm = gt_rot6d - gt_rot6d.mean(dim=1, keepdim=True)
        loss_tvar = masked_loss(pred_dm, gt_dm, rot_joint_mask_for_loss,
                                nn.L1Loss(reduction='none'))
        loss_dict["loss_tvar"] = loss_tvar
        total_loss = total_loss + weight_cfg["tvar_wt"] * loss_tvar
