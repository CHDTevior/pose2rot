...
class AnySpeciesPoseDataset(Dataset):
    def __init__(
        ...
        # meta cache
        # =========================================================
        # split_tag keys the cache by the split_json identity so different held-out
        # splits (e.g. empty all-data vs seen/rare/unseen) never share a cache and leak
        # test motions into training. Was: filename only had split_mode/split_group/window.
        _split_tag = os.path.splitext(os.path.basename(split_json))[0] if split_json else "nosplit"
        self.items_cache_path = os.path.join(
            root,
            f"cache/__mesh2pose1002_{self.split_mode}_{self.split_group}_{_split_tag}_seqlen_{self.window}_items_cache.json"
        )
        ...
        self._fk_offset_cache = {}  # species -> parent-local bvh offsets/gscale, for FK-position loss
    ...
    # preload all sequences into RAM
    # ============================================================
    def _fk_offset_for(self, species_name, bvh_fpath, gscale, J):
        """Parent-local BVH bone offsets / gscale (NO root-subtract), padded to [1, MAX_JOINTS, 3],
        for the FK-position loss. Matches the validated FK in pose2rot_qa.py (bvh.load offsets, NOT
        the root-subtracted offset_a). Cached per species (skeleton is per-species constant)."""
        if species_name not in self._fk_offset_cache:
            off = np.asarray(_bvh.load(bvh_fpath)[0].offsets, dtype=np.float32) / gscale  # [Jb, 3]
            self._fk_offset_cache[species_name] = off
        off = self._fk_offset_cache[species_name]
        fk = np.zeros((1, MAX_JOINTS, 3), dtype=np.float32)
        n = min(int(J), off.shape[0], MAX_JOINTS)
        fk[0, :n] = off[:n]
        return fk
    ...
    def _preload_all_data(self) -> List[Dict[str, Any]]:
        ...
        for it in iterator:
            ...
            data_cache.append({
                ...
                "parent_a": parents,
                "offset_a": rest_pose,
                "fk_offset": self._fk_offset_for(species_name, bvh_fpath, gscale, J),
            })
...
def collate_anyspecies_padded(batch):
    ...
    for b in batch:
        ...
        fk_offset_list.append(b["fk_offset"].squeeze(0)[:J_max])
    ...
    return {
        ...
        "fk_offset": torch.from_numpy(np.stack(fk_offset_list, 0)),
