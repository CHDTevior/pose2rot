...
def get_scale_cached(bvh_pth, auto_scale='base_mesh'):
    ...
    elif auto_scale == 'none':
        # raw units: position == position_before (no pre-scaling). Use this so
        # bvh_pose 'position' stays in the same units as the BVH rest offsets the
        # loader reads as offset_a, keeping (position/gscale) and (offset_a/gscale)
        # self-consistent (loader_v2.py:416/480 divide both by the SAME gscale).
        scale = 1.0
    else:
        scale = 1.0
